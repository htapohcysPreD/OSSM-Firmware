...
class Pattern
{
    ...
    */
    virtual void setStroke(int stroke)
    {
        _stroke = stroke;
    }
    ...
    */
    virtual void setSensation(float sensation)
    {
    ...
    */
    ...
    /*!
      ...
      @param stepsPerMM
    */
    virtual void setSpeedLimit(unsigned int maxSpeed, unsigned int maxAcceleration, unsigned int stepsPerMM)
    {
        _maxSpeed = maxSpeed;
...
#ifdef DEBUG_PATTERN
        Serial.println("amplitude: " + String(amplitude) + " cycleIndex: " + String(cycleIndex));
#endif
...
class JackHammer : public Pattern
{
   public:
    ...
    motionParameter nextTarget(unsigned int index)
    {
        ...
        // only calculate new position, if index has incremented: no mid-stroke update, as vibration is sufficiently
        // fast except for return stroke if depth is exceeded.
        if (index != _index)
        {
   ...
   protected:
    ...
    void _updateVibrationParameters()
    {
        ...
        _inVibrationDistance =
            (int)fscale(-100.0, 100.0, (float)(3.0 * _stepsPerMM), (float)(25.0 * _stepsPerMM), _sensation, 0.0);
...
#ifdef DEBUG_PATTERN
        Serial.println("_maxSpeed: " + String(_maxSpeed) + " _strokeInSpeed: " + String(_strokeInSpeed) +
                       " _strokeOutSpeed: " + String(int(1.5 * _stroke / _timeOfStroke)));
